...
const filtroActivo = document.getElementById('etiquetaInfo');
...
// Mensaje de búsqueda (error abajo del grid)
...
appGrid.parentNode.insertBefore(limpiarBtn, message);
  ...
  .then(data => {
    ...
    document.querySelectorAll('.tag').forEach(tagElement => {
      tagElement.addEventListener('click', function (e) {
        ...
        filtroActivo.textContent = `Resultados para la etiqueta "${tag}" (${matches} resultado${matches !== 1 ? 's' : ''})`;
    ...
    searchInput.addEventListener('input', function () {
      ...
      filtroActivo.textContent = rawQuery
        ? `Buscando: ${rawQuery} (${matches} resultado${matches !== 1 ? 's' : ''})`
        : '';
...
function filtrarCategoria(categoria) {
  ...
  filtroActivo.textContent = `Filtrando por categoría: ${categoria} (${matches} resultado${matches !== 1 ? 's' : ''})`;
  limpiarBtn.style.display = 'inline-block';
  searchInput.value = '';
